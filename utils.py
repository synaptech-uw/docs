...
def compute_emg_powers(eegdata, fs):
    """Extract the features (band powers) from the EEG.
    ...
    # Flex1 2-10
    ind_flex1, = np.where((f >= 2) & (f <= 10))
    meanflex1 = np.mean(PSD[ind_flex1, :], axis=0)
    # Flex2 50-80
    ind_flex2, = np.where((f >= 30) & (f < 80))
    meanflex2 = np.mean(PSD[ind_flex2, :], axis=0)

    feature_vector = np.concatenate((meanflex1, meanflex2), axis=0)
